...
var xModal = {
	 ...
	 */
	// openAdj : function(_item,_title,_size,_isIframe,_iframeHeight,_footer){
	// 	var content = '';
	// 	var size = '';
	// 	var footer = '';
	// 	var widthModal = '';
	...
	open : function(_item,_title,_size,id,_isIframe,_iframeHeight,_footer){
		...
		var titulo = '<h5 class="modal-title">&nbsp;</h5>';
		if(_title !== undefined){
			titulo = '<h5 class="modal-title">'+_title+'</h5>';
		}
		...
		var innerModal = '<div class="modal fade" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="overflow-y:scroll;" id="'+id+'">'+
							  		 '<div class="modal-dialog '+size+' modal-lg-xs" style="margin-top:10px; '+widthModal+'">'+
	 ...
	 * @return {[type]} [description]
	 */
	success : function(_msg,_callback,_doc=null){
		...
		if(_doc != null) doc = _doc;

		var innerModal = '<div class="modal fade" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" id="modal-success">'+
							    ...
							    '<div class="modal-content panel-success">'+
							      '<div class="modal-header panel-heading" >'+
								     ...
								     '<h5 class="modal-title"><span class="glyphicon glyphicon-ok-sign"></span> <strong>Exito</strong></h5>'+
									  ...
									  '<button type="button" class="btn btn-success" onclick="xModal.close(\'modal-success\')" id="btn-aceptarAlerta">Aceptar</button>'+
									  doc + '</div>'+
							    '</div>'+
							  '</div>'+
							'</div>';
	...
	confirm : function(_msg,_callback, _callback2){
		...
		var innerModal = '<div class="modal fade" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" id="modal-confirm">'+
							  '<div class="modal-dialog" style="margin-top:16px;">'+
							    '<div class="modal-content panel-primary" >'+
							      '<div class="modal-header panel-heading" >'+
								     ...
								     '<h5 class="modal-title"><span class="glyphicon glyphicon-question-sign"></span> <strong>Confirmación</strong></h5>'+
							      	...
							      	'<button type="button" style="margin-bottom: 5px;" class="btn btn-primary" onclick="xModal.close();" id="btn-siConfirm">Si</button></div>'+
							    '</div>'+
							  '</div>'+
							'</div>';
	 ...
	 * @return {[type]} [description]
	 */
	info : function(_msg,_callback,_doc=null){
		var backLast 	= $(".modal-backdrop").last().css('z-index');
		var modalLast 	= $(".modal").last().css('z-index');
		var doc 		= '';
		if(backLast !== undefined){
		...
		var innerModal = '<div class="modal fade" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" id="modal-info">'+
							  '<div class="modal-dialog" style="margin-top:16px; ">'+
							    '<div class="modal-content panel-info" style="width:586px">'+
							      '<div class="modal-header panel-heading" >'+
								     ...
								     '<h5 class="modal-title"><span class="glyphicon glyphicon-info-sign"></span> <strong>Información</strong></h5>'+
									  ...
									  '<div class="modal-body" style="overflow:hidden" id="content-modal">'+_msg+'</div>'+
										'<div class="modal-footer">'+
										doc +'<button type="button" class="btn btn-default" onclick="xModal.close()" id="btn-aceptarAlerta">Aceptar</button>'+
										'</div>'+
							    '</div>'+
							  '</div>'+
							'</div>';
	...
	danger : function(_msg,_callback){
		...
		/**
	 * Alerta para mensajes de errores con confirmación
	 * @return {[type]} [description]
	 */
	dangerConfirm : function(_msg,_msg_title,_callback,_callback2){
		var backLast = $(".modal-backdrop").last().css('z-index');
		var modalLast = $(".modal").last().css('z-index');
		if(backLast !== undefined){
			backLast = parseInt(modalLast) + 1;
		...
		var innerModal = '<div class="modal fade" id="modal-error" aria-labelledby="myLargeModalLabel2" aria-hidden="true">'+
							  '<div class="modal-dialog" style="margin-top:16px;">'+
							    '<div class="modal-content panel-danger">'+
							      '<div class="modal-header panel-heading" >'+
								     '<h5 class="modal-title"><span class="glyphicon glyphicon-remove-sign"></span> <strong>'+_msg_title+'</strong></h5>'+
	      							'</div>'+
											'<div class="modal-body" style="overflow:hidden" id="content-modal">'+_msg+'</div>'+
							      	'<div class="modal-footer"><button type="button" class="btn btn-default" onclick="xModal.close();" id="btn-noConfirm">Cancelar</button>'+
							      	'<button type="button" class="btn btn-danger"  style="margin-bottom: 5px;" onclick="xModal.close(\'modal-error\')" id="btn-aceptarAlerta"><strong>Aceptar</strong></button></div>'+
							    '</div>'+
							  '</div>'+
							'</div>';
	...
	warning : function(_msg, _callback){
		...
		var innerModal = '<div class="modal fade" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" id="modal-warning">'+
							  '<div class="modal-dialog" style="margin-top:16px;">'+
